...
# 테이블 이름 설정
store_table_name = "popup"
store_image_table_name = "popup_image"
store_review_table_name = "popup_review"
...
def store_data(num_rows):
    ...
    try:
        ...
        for i in range(1, num_rows + 1):
            ...
            status = random.choice(["POPUP_END","POPUP_RESERVE","POPUP_START","POPUP_STOCK"])
            ...
            if len(insert_values) >= batch_size:
                cursor.executemany(f"""
                    INSERT INTO {store_image_table_name} (
                        created_at, updated_at, url, popup_idx
                    ) VALUES (%s, %s, %s, %s);
                """, insert_values)
                  # 1000개 삽입 후 출력
                insert_values = []  # 한 번에 삽입 후 초기화
...
def store_review_data(num_rows):
    ...
    try:
        ...
        for i in range(1, num_rows + 1):
            ...
            popup_idx = random.randint(1, 10000)
            ...
            if len(insert_values) >= batch_size:
                cursor.executemany(f"""
                    INSERT INTO {store_review_table_name} (
                        ...
                        customer_name, rating, title, customer_idx, popup_idx
