...
public class ReserveService {
    ...
    public ReserveDto.CreateReserveRes registerReserve(CustomUserDetails customUserDetails, ReserveDto.CreateReserveReq dto) throws BaseException {
        ...
        // 스토어 소유 확인
        if(!Objects.equals(store.getCompanyEmail(), customUserDetails.getEmail())) throw new BaseException(BaseResponseMessage.RESERVE_REGISTER_FAIL_INVALID_MEMBER);

        // 스토어 최대 예약자 수를 넘었는지 확인
        if(store.getTotalPeople() <= 0) throw new BaseException(BaseResponseMessage.RESERVE_REGISTER_FAIL_LIMIT_EXCEEDED);
    ...
    public ReserveDto.EnrollReserveRes enrollReserve(HttpServletResponse res, CustomUserDetails customUserDetails, Long reserveIdx) throws BaseException {
        ...
        Long currentOrder = redisUtil.getOrder(reserve.getWorkingUUID(), customUserDetails.getEmail());
        Long workingQueueSize = redisUtil.getSize(reserve.getWorkingUUID());
        long rank;
        String response = "";
        ...
        if (workingQueueSize < reserve.getTotalPeople()) {
            // 새로운 사용자 등록
            redisUtil.enrollQueue(reserve.getWorkingUUID(), customUserDetails.getEmail(), System.currentTimeMillis());
            issueWToken(res, reserveIdx, customUserDetails.getEmail());
            rank = redisUtil.getOrder(reserve.getWorkingUUID(), customUserDetails.getEmail()) + 1;
            response = "예약 접속 -> 접속 번호: " + rank;
        }

        // 예약 접속 큐가 꽉 찬 경우 -> 대기 큐에 추가
        if (workingQueueSize >= reserve.getTotalPeople()) {
            redisUtil.enrollQueue(reserve.getWaitingUUID(), customUserDetails.getEmail(), System.currentTimeMillis());
            rank = redisUtil.getOrder(reserve.getWaitingUUID(), customUserDetails.getEmail()) + 1;
    ...
    public String cancelReserve(HttpServletResponse res,CustomUserDetails customUserDetails, Long reserveIdx) throws BaseException {
        ...
        if (currentOrder != null) {
            ...
            redisUtil.remove(reserve.getWorkingUUID(), customUserDetails.getEmail());
            deleteWToken(res);
            String firstWaitingUser = redisUtil.firstWaitingUserToWorking(reserve.getWorkingUUID(), reserve.getWaitingUUID(), reserve.getTotalPeople());
            if(firstWaitingUser != null ) log.info("대기자에서 예약자로 이동: {}", firstWaitingUser);
            else log.info("대기자가 없습니다.");
        } else {
            Long waitingOrder = redisUtil.getOrder(reserve.getWaitingUUID(), customUserDetails.getEmail());
            if (waitingOrder != null) {
                // 대기 큐에서 현재 사용자 제거
                redisUtil.remove(reserve.getWaitingUUID(), customUserDetails.getEmail());
    ...
    public Page<ReserveDto.SearchReserveRes> searchAllReserve(Long storeIdx, String keyword, int page, int size) throws BaseException {
        ...
        // 예외: Store 조회 결과 없음
        if (reservePage.isEmpty()) throw new BaseException(BaseResponseMessage.RESERVE_SEARCH_ALL_FAIL_NOT_FOUND);
